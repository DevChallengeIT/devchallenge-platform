# frozen_string_literal: true

module UI
  H2 = 'text-2xl font-semibold'
  H3 = 'text-xl font-semibold'
  H4 = 'text-md font-semibold'
  FORM_LABEL = 'block text-sm font-bold mb-2 text-gray-700'
  FORM_INPUT = 'shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline'
  BUTTON_PRIMARY = 'rounded py-3 px-5 bg-blue-600 text-white block font-medium'
  BUTTON_SECONDARY = 'rounded py-3 px-5 bg-gray-100 inline-block font-medium'
  BUTTON_DANGER = 'rounded bg-red-700 hover:bg-red-900 text-white font-bold py-3 px-5 focus:outline-none focus:shadow-outline'
  PANEL = 'h-full w-full max-w-xs mx-auto align-middle'
  LINK = 'align-baseline font-bold text-sm text-blue-500 hover:text-blue-800'
end
